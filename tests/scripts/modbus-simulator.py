...
def update_values_continuously(context):
    """持续更新模拟数据"""
    print("开始数据更新线程...")
    ...
    while True:
        try:
            ...
            # 模拟动态数据变化
            slave_context = context[1]  # 获取单元ID=1的上下文
...
def main():
    ...
    # 启动数据更新线程
    update_thread = threading.Thread(target=update_values_continuously, args=(context,), daemon=True)
    update_thread.start()
