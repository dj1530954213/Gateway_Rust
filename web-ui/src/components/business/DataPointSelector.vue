<template>
  <div class="data-point-selector" :class="containerClass">
          ...
          <template #default="{ node, data }">
            <div class="tree-node-content" :class="getNodeClass(data)">
              ...
              <div class="node-info">
                ...
                <div
                  v-if="showNodeDetails && data.type === 'datapoint'"
                  class="node-details"
                >
                  ...
                  <span v-if="data.unit" class="detail-item"
                    >单位: {{ data.unit }}</span
                  >
                </div>
              </div>
    ...
    <div v-if="multiple && selectedCount > 0" class="selector-batch-actions">
      <div class="batch-info">已选择 {{ selectedCount }} 个数据点</div>
    ...
    >
      <div v-if="previewDataPoint" class="data-preview">
        <el-descriptions border column="1">
          ...
          <el-descriptions-item label="数据质量">
            <StatusTag
              :status="previewDataPoint.quality"
              :mappings="qualityMappings"
            />
          </el-descriptions-item>
          ...
          <el-descriptions-item
            v-if="previewDataPoint.description"
            label="描述"
          >
      ...
      <template #footer>
        ...
        <el-button
          v-if="!isSelected(previewDataPoint)"
          type="primary"
          @click="handleSelectFromPreview"
        >
          选择此数据点
        </el-button>
      </template>
...
const selectedCount = computed(() => {
  return props.multiple
    ? (selectedItems.value as string[]).length
    : selectedItems.value
      ? 1
      : 0
})
  ...
  return nodes.filter(node => {
    ...
    if (node.type === 'datapoint') {
      ...
      if (searchKeyword.value) {
        ...
        const searchText =
          `${node.name} ${node.address || ''} ${node.description || ''}`.toLowerCase()
