            ...
            <div class="card-content">
              ...
              <div class="card-info">
                ...
                <div class="card-value">
                  {{ formatNumber(overviewData.totalPoints) }}
                </div>
          ...
          <el-card class="overview-card" shadow="hover">
            <div class="card-content">
              ...
              <div class="card-info">
                ...
                <div class="card-value">
                  {{ formatNumber(overviewData.processRate) }}/s
                </div>
                <div class="card-trend positive">
                  <el-icon><CaretTop /></el-icon>
                  <span>+8.1%</span>
...
async function initializeData() {
  try {
    ...
    // 并行加载各种数据
    await Promise.all([loadOverviewData(), loadChartData(), loadTableData()])
...
async function executeAnalysis() {
  try {
    ...
    // 重新加载数据
    await Promise.all([loadChartData(), loadTableData()])
...
watch(
  ...
  () => {
    if (
      analysisForm.value.dateRange &&
      analysisForm.value.dateRange.length === 2
    ) {
      loadChartData()
    }
  }
