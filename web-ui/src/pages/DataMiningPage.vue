        ...
        <el-card shadow="never">
          <div class="nav-items">
            ...
            >
              ...
              <div v-if="func.badge" class="nav-badge">
                <el-badge
                  :value="func.badge"
                  :type="func.badgeType || 'primary'"
                />
              </div>
            </div>
          </div>
        ...
        <el-table :data="filteredModels" stripe style="width: 100%">
          ...
          <el-table-column label="操作" width="200">
            <template #default="{ row }">
              ...
              <el-button size="small" @click="evaluateModel(row)"
                >评估</el-button
              >
              <el-dropdown @command="cmd => handleModelAction(cmd, row)">
                ...
                <template #dropdown>
                  <el-dropdown-menu>
                    ...
                    <el-dropdown-item command="retrain"
                      >重新训练</el-dropdown-item
                    >
...
const filteredTemplates = computed(() => {
  ...
  return analysisTemplates.value.filter(
    t => t.category === selectedTemplateCategory.value
  )
})
...
function deployModel(model: any) {
  ElMessageBox.confirm(`确定要部署模型 "${model.name}" 吗？`, '部署确认', {
...
function handleModelAction(command: string, model: any) {
  switch (command) {
    ...
    case 'retrain':
      ...
      break
    case 'delete':
      ElMessageBox.confirm(`确定要删除模型 "${model.name}" 吗？`, '删除确认', {
