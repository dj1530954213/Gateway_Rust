...
http.interceptors.response.use(
  (response: AxiosResponse) => {
    ...
    // 调试日志
    if (import.meta.env.VITE_DEBUG === 'true') {
      console.group(`✅ API响应: ${response.status} ${config.method?.toUpperCase()} ${config.url}`)
      console.log('Request ID:', requestId)
  ...
  async (error) => {
    ...
    // 调试日志
    if (import.meta.env.VITE_DEBUG === 'true') {
      console.group(`❌ API错误: ${response?.status || 'Network'} ${config?.method?.toUpperCase()} ${config?.url}`)
      console.log('Request ID:', requestId)
...
// 文件下载
export async function download(url: string, filename?: string, params?: any): Promise<void> {
  try {
