  ...
  <div class="connectors-page">
    ...
    <div class="toolbar">
      ...
      <div class="toolbar-right">
        <el-dropdown @command="handleBatchAction" :disabled="!selectedConnectors.length">
      ...
      >
        ...
        <template #config="{ row }">
          <div class="config-cell">
            ...
            <span class="config-count">{{ Object.keys(row.config).length }} 项配置</span>
          </div>
        </template>
        ...
        <template #metrics="{ row }">
          <div class="metrics-cell">
            <div class="metric-item">
              ...
              <span class="metric-value">{{ formatNumber(row.messagesSent) }}</span>
            </div>
            <div class="metric-item">
    ...
    <div v-else class="grid-view">
      <el-row :gutter="16">
        <el-col :span="8" v-for="connector in filteredConnectors" :key="connector.id">
          <el-card shadow="hover" class="connector-card" :class="getCardClass(connector)">
            <template #header>
              <div class="card-header">
                <div class="connector-info">
                  <el-icon class="connector-icon" :class="getTypeIconClass(connector.type)">
                    <component :is="getTypeIcon(connector.type)" />
                  </el-icon>
                  <div class="connector-basic">
                    <h4 class="connector-name">{{ connector.name }}</h4>
                    <span class="connector-type">{{ getTypeLabel(connector.type) }}</span>
                  </div>
                </div>
                ...
                <div class="card-actions">
                  ...
                  <el-dropdown @command="(cmd) => handleRowAction(cmd, connector)" trigger="click">
            ...
            <div class="card-content">
              ...
              <div class="connection-info">
                <div class="info-item">
                  ...
                  <span class="info-value">{{ getConnectionTarget(connector) }}</span>
                </div>
                <div class="info-item">
                  <span class="info-label">协议:</span>
                  <span class="info-value">{{ connector.protocol || 'N/A' }}</span>
                </div>
                <div class="info-item">
                  <span class="info-label">创建时间:</span>
                  <span class="info-value">{{ formatTime(connector.createdAt) }}</span>
                </div>
              </div>
              ...
              <div class="performance-metrics">
                <div class="metrics-grid">
                  <div class="metric-box">
                    <div class="metric-number">{{ formatNumber(connector.messagesSent) }}</div>
                    <div class="metric-title">消息发送</div>
                  </div>
                  <div class="metric-box">
                    <div class="metric-number error">{{ connector.errorCount }}</div>
                    <div class="metric-title">错误计数</div>
                  </div>
                  <div class="metric-box">
                    <div class="metric-number">{{ connector.avgLatency }}ms</div>
                    <div class="metric-title">平均延迟</div>
                  </div>
                  <div class="metric-box">
                    <div class="metric-number">{{ connector.uptime || 0 }}h</div>
              ...
              <div class="status-info">
                <div class="status-indicators">
                  <div class="indicator">
                    <el-tag size="small" :type="getConfigStatusType(connector.configValid)">
                  ...
                  <div class="indicator" v-if="connector.lastError">
      ...
      <div class="test-result">
        ...
        <div class="result-details">
          <el-descriptions :column="1" border>
            <el-descriptions-item label="测试时间">{{ formatTime(testResult.timestamp) }}</el-descriptions-item>
...
const getOverallStatusType = () => {
  ...
  const hasDisconnected = connectors.value.some(conn => conn.status === 'disconnected')
...
const getOverallStatusText = () => {
  const connectedCount = connectors.value.filter(conn => conn.status === 'connected').length
